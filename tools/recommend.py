...
def recommend_communities(user_nodes, community_graph, df_skill):
    status = ""
    messages = ""
    maintype = []
    percent = []
    ...
    for community_id in set(partition_louvain_slpa.values()):
        nodes = [
            node
            for node, node_label in partition_louvain_slpa.items()
            if node_label == community_id
        ]
    ...
    total_percentage = sum(
        user_percentage
        for _, _, user_percentage in target_communities[: len(target_communities)]
    )
    ...
    updated_communities = []
    for i, (community_id, nodes, user_percentage) in enumerate(
        target_communities[: len(target_communities)]
    ):
        updated_percentage = (
            user_percentage + (user_percentage / total_percentage) * diff_percentage
        )
    ...
    if updated_communities:
        ...
        for i in range(len(maintype)):
            ...
            result = {"status": status, "messages": messages, "courses": courses}
...
def handle_recomment(user_node):
    G = nx.Graph(day="Elearning")  # Khởi tạo đồ thị

    df_nodes = pd.read_csv(
        f"{current_path}/Dataset/StackOverFlow/link_node_stack_v2.csv"
    )
    df_edges = pd.read_csv(
        f"{current_path}/Dataset/StackOverFlow/link_edges_stack_v2.csv"
    )
    seed_node = df_nodes["name"].tolist()
    ...
    # =============================
    df_skill = pd.read_csv(f"{current_path}/Dataset/Roadmap/Skill.csv")
    community_graph = G.copy()
    # =====
    result = recommend_communities(user_node, community_graph, df_skill)
    return result
